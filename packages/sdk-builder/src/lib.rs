...
mod tests {
    ...
    #[test]
    fn test_ffmpeg_compiler() {
        // 设置源码和构建目录
        let current_dir = env::current_dir().unwrap();
        let tmp_dir = current_dir.join("tmp");

        // 使用 FFmpegBuilder 配置编译选项
        let ffmpeg_builder = FFmpegBuilder::new()
            .source_dir(&tmp_dir)
            .build_dir(tmp_dir.join("ffmpeg_build"))
            // 添加必要的组件
            ...
            // 添加音频编解码器
            .with_audio_codecs([AudioCodec::Opus, AudioCodec::AAC])
            // 添加封装格式
            .with_muxer_formats([
        ...
        // 构建编译器
        let compiler = ffmpeg_builder.build();

        // 开始编译，并获取编译输出
        let compile_output = compiler.compile().unwrap();
